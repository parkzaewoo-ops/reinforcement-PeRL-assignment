...
3. **의사 결정 (Decider)**
본 프로젝트의 보상(Reward)은 생성된 SQL의 정확도를 기반으로 정량화됩니다. 다만 보상 함수는 실험 방식(Execution Match, LLM Judge, Token Similarity 등)에 따라 **서로 다르게 설계될 수 있으며**, 현재 적용된 보상 구조는 **여러 가능한 설계 중 하나의 예시적 구성**입니다.  

   - 보상 ≥ 1.0 → 성공 처리  
   - 보상 < 1.0 → 실패 처리 후 프롬프트 개선  
   - 실패 3회 이상 → 조기 중단 및 다음 데이터로 이동

5. **프롬프트 업데이트 (Update Prompt)**  
